...
show_help() {
  cat <<'HELP'
Usage: run_revision_val_inference.sh --data-root DATA_ROOT --checkpoint-root CHECKPOINT_ROOT --output-root OUTPUT_ROOT [--dry-run]

Revision-validation inference only. Does not access official test.
HELP
}
...
case "run_revision_val_inference" in
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  echo "DRY RUN: run_revision_val_inference would execute with: ${ARGS[*]:-<no args>}"
  exit 0
fi
