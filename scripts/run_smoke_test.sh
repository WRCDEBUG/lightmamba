...
show_help() {
  cat <<'HELP'
Usage: run_smoke_test.sh [--dry-run] [--help]

Fast verification only. No dataset, no detector training, no SAM inference.
HELP
}
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  echo "DRY RUN: run_smoke_test would execute with: ${ARGS[*]:-<no args>}"
  exit 0
fi
