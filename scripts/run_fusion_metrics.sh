...
show_help() {
  cat <<'HELP'
Usage: run_fusion_metrics.sh --data-root DATA_ROOT --fused-root FUSED_ROOT --output-root OUTPUT_ROOT [--dry-run]

Computes luminance-domain source-referenced fusion metrics.
HELP
}
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  echo "DRY RUN: run_fusion_metrics would execute with: ${ARGS[*]:-<no args>}"
  exit 0
fi
