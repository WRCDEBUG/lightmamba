...
show_help() {
  cat <<'HELP'
Usage: run_detection_evaluation.sh --data-root DATA_ROOT --checkpoint-root CHECKPOINT_ROOT --output-root OUTPUT_ROOT [--official-test --acknowledge-frozen-official-test] [--dry-run]

Uses frozen detector metadata. Does not train detector.
HELP
}
...
case "run_detection_evaluation" in
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  echo "DRY RUN: run_detection_evaluation would execute with: ${ARGS[*]:-<no args>}"
  exit 0
fi
